...
const BaileysQRModal: React.FC<BaileysQRModalProps> = ({
  ...
  const startWhatsAppSession = useCallback(async () => {
    ...
    try {
      console.log(`Iniciando sessão WhatsApp para conexão ${connectionId}`);
      const response = await fetch(`/api/proxy`, {
        ...
        body: JSON.stringify({
          url: `${apiUrl}/whatsappsession/${connectionId}`,
          method: 'POST',
          headers: {
  ...
  }, [connectionId, apiUrl, authToken]);

  // Verificar se a sessão já está iniciada ao abrir o modal
  useEffect(() => {
    if (isOpen && connectionId && !sessionStarted) {
      startWhatsAppSession();
    }
  }, [isOpen, connectionId, sessionStarted, startWhatsAppSession]);
  ...
  }, [isOpen, connectionId, fetchQRCode]);
  ...
  const checkConnectionStatus = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/proxy`, {
        ...
        body: JSON.stringify({
          url: `${apiUrl}/whatsapp/status/${connectionId}`,
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${authToken}`,
            'Accept': 'application/json'
      ...
      console.log(`Status da conexão ${connectionId}:`, data);
      ...
      // Se estiver conectado, mantém com ping periódico
        ...
        await fetch(`/api/proxy`, {
          ...
          body: JSON.stringify({
            url: `${apiUrl}/whatsapp/ping/${connectionId}`,
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${authToken}`,
              'Accept': 'application/json'
  ...
  }, [connectionId, apiUrl, authToken, isOpen, onClose, startWhatsAppSession]);
  ...
  }, [connectionId, checkConnectionStatus]);
  ...
  const reconnectAndGetNewQRCode = useCallback(async () => {
    ...
    try {
      ...
      if (connectionId) {
        console.log(`Desconectando a sessão atual para conexão ${connectionId}`);
        await fetch(`/api/proxy`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          ...
          body: JSON.stringify({
            url: `${apiUrl}/whatsapp/disconnect/${connectionId}`,
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${authToken}`,
              'Accept': 'application/json'
  ...
  }, [connectionId, apiUrl, authToken, startWhatsAppSession, fetchQRCode]);
